...
public class Rating extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loading=new ProgressDialog(Rating.this);
        recyclerView=(RecyclerView)findViewById(R.id.listmenurating);
        recyclerView.setHasFixedSize(true);
        GridLayoutManager gridLayout=new GridLayoutManager(Rating.this,1);
...
//        Tampil_Kosan();
    ...
    private void Tampil_Kosan(double latitude, double longitude) {
        ...
        Call<Response_Kosan> menuCall = api.Rating(latitude,longitude);
        menuCall.enqueue(new Callback<Response_Kosan>() {
            ...
            public void onResponse(Call<Response_Kosan> call, Response<Response_Kosan> response) {
                if (response.isSuccessful()){
                    ...
                    if (status){
                        loading.dismiss();
                        Adapter_Terdekat adapter = new Adapter_Terdekat(Rating.this, device);
                        recyclerView.setAdapter(adapter);
                    } else {
                        try {
                            loading.dismiss();
                            Toast.makeText(Rating.this, "Tidak Ada data Menu saat ini", Toast.LENGTH_SHORT).show();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
...
//                    Toast.makeText(getActivity(), "Server Tidak Merespon", Toast.LENGTH_SHORT).show();
    ...
    private void Back() {
        Intent intent=new Intent(Rating.this,Menu_Utama.class);
