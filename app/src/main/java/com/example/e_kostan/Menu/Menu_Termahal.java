...
public class Menu_Termahal extends AppCompatActivity {
    RecyclerView recyclerView;
    ProgressDialog loading;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_menu__termahal);
        recyclerView = (RecyclerView) findViewById(R.id.listhargatermahal);
        loading=new ProgressDialog(Menu_Termahal.this);
        recyclerView.setHasFixedSize(true);
        GridLayoutManager gridLayout=new GridLayoutManager(Menu_Termahal.this,1);
...
//        Tampil_Kosan();
    ...
    private void Tampil_Kosan(double latitude, double longitude) {
        ...
        Call<Response_Kosan> menuCall = api.Tampil_mahal(latitude,longitude);
        menuCall.enqueue(new Callback<Response_Kosan>() {
            ...
            public void onResponse(Call<Response_Kosan> call, Response<Response_Kosan> response) {
                if (response.isSuccessful()){
                    ...
                    if (status){
                        loading.dismiss();
                        Adapter_Terdekat adapter = new Adapter_Terdekat(Menu_Termahal.this, device);
                        recyclerView.setAdapter(adapter);
                    } else {
                        try {
                            loading.dismiss();
                            Toast.makeText(Menu_Termahal.this, "Tidak Ada data Menu saat ini", Toast.LENGTH_SHORT).show();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
...
//                    Toast.makeText(getActivity(), "Server Tidak Merespon", Toast.LENGTH_SHORT).show();
    ...
    private void Back() {
        Intent intent=new Intent(Menu_Termahal.this,Menu_Utama.class);
